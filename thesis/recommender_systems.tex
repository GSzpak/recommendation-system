...
\section{Wyznaczanie zbioru s±siadów}
...
Poni¿sze dwa wykresy przedstawiaj± zale¿no¶æ MAE oraz RMSE od warto¶ci $k$ dla obu
...
Ostatnim krokiem w algorytmie wspólnej filtracji jest ustalenie oceny $r_{ui}$ na podstawie
ocen s±siadów ze zbioru $N$. 
...
dla s±siadów. Naturalnym pomys³em jest zatem zast±pienie ¶redniej arytmetycznej ¶redni± wa¿on± (w tej sekcji podawana bêdzie tylko wersja oparta na u¿ytkownikach, wzory dla
filtracji bazuj±cej na u¿ytkownikach s± analogiczne):
\begin{equation} \label{result:w_mean}
    r_{ui} = \frac{\sum_{v \in N_i(u)} s_{users}(u, v) r_{vi}}{\sum_{v \in N_i(u)} |s_{users}(u, v)|}
\end{equation}

\subsection{Normalizacja wzglêdem ¶redniej}

\subsection{Standaryzacja Z}
