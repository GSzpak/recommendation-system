...
\section{Filtracja w oparciu o u¿ytkowników}
   ...
   \textit{GroupLens Research Project}
   \footnote{Projekt badawczy prowadzony na Uniwersytecie Minnesota}
   [TODO bibliografia].
...
\subsubsection{Miary podobieñstwa bazuj±ce na korelacji}
 \subsubsection{Inne miary podobieñstwa}
    ...
    \begin{equation}
        spearman\_corr(u, v) = \frac{\sum_{i \in I_u \cap I_v}(k_{ui} - \mean{k}_u)(k_{vi} - \mean{k}_v)}{\sqrt{\sum_{i \in I_u \cap I_v}(k_{ui} - \mean{k}_u)^2}\sqrt{\sum_{i \in I_u \cap I_v}(k_{vi} - \mean{k}_v)}}
    \end{equation}
...
\begin{equation}
adjusted\_cosine\_sim(i, j) = \frac{\sum_{u \in U_i \cap U_j}(r_{ui} - \mean{r}_u)(r_{uj} - \mean{r}_u)}{\sqrt{\sum_{u \in U_i \cap U_j}(r_{ui} - \mean{r}_u)^2}\sqrt{\sum_{u \in U_i \cap U_v}(r_{uj} - \mean{r}_u)}} \label{sim:adjusted_cos}
\end{equation}
...
\chapter{Wp³yw miary podobieñstwa na jako¶æ predykcji}
