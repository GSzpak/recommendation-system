   ...
   \textit{GroupLens Research Project}\footnote{Projekt badawczy prowadzony na Uniwersytecie Minnesota} (¼ród³o: \cite{ekstr}).
    ...
    (linia \ref{alg:result}) jest z kolei wziêcie ¶redniej oceny ze zbioru s±siadów:
     \begin{equation} \label{result:mean}
        \hat{r}_{ui} = \frac{1}{|N_i(u)|} \sum_{v \in N_i(u) r_{vi}} r_{vi}
    \end{equation}
...
Odpowiedni dobór parametru $k$ mo¿e wyra¼nie poprawiæ jako¶æ rekomendacji. Poni¿sze dwa wykresy przedstawiaj± zale¿no¶æ MAE oraz RMSE od warto¶ci $k$ dla obu
metod wspólnej filtracji. W obu przypadkach wybrano miary, które dawa³y najlepsze wyniki dla
$k =10$ - w przypadku podej¶cia bazuj±cego na u¿ytkownikach by³y to TODO oraz TODO,
a w przypadku podej¶cia opartego na przedmiotach - TODO oraz TODO.
...
Poni¿sze dwie tabele przedstawiaj± wyniki otrzymane przez zastosowanie wy¿ej
wymienionych metod. 
...
\captionsetup{justification=centering}
\captionsetup{width=0.8\textwidth}
\caption{Wyniki dla podej¶cia opartego na u¿ytkownikach przy $k = 30$ i $s_{users} = pearson\_corr$}
\begin{tabular}{|c|c|c|}
\hline
                         & MAE & RMSE \\ \hline
¶rednia wa¿ona          & 0.749   & 0.959    \\ \hline
$mean-centering$ & 0.779   & 0.998    \\ \hline
$z-score$   & 0.764   &  0.976   \\ \hline
\end{tabular}
\end{minipage}
\hfillx
\begin{minipage}{0.5\textwidth}
...
\captionsetup{justification=centering}
\captionsetup{width=0.8\textwidth}
\caption{Wyniki dla podej¶cia opartego na przedmiotach przy $k = 30$ i $s_{items} = pearson\_corr$}
\begin{tabular}{|c|c|c|}
\hline
                         & MAE & RMSE \\ \hline
¶rednia wa¿ona          & 0.749   & 0.959    \\ \hline
...
Parametry algorytmu - liczba s±siadów $k$ oraz funkcja podobieñstwa - zosta³y dobrane tak, aby ostatecznie otrzymaæ jak najlepsz± jako¶æ predykcji.
Dla podej¶cia bazuj±cego na u¿ytkownikach by³y to $s_{users} = $ i $k=$, a dla
podej¶cia opartego na przedmiotach: $s_{items} = $ i $k = $ TODO. 

Jak widaæ, zastosowanie obu typów normalizacji (wzglêdem ¶redniej oraz standaryzacji Z)
znacznie poprawi³o jako¶æ predykcji - przyk³adowo, $MAE$ dla podej¶cia opartego na
przedmiotach zmala³ o ponad 10\% w stosunku do predyktora bazowego opisanego w
rozdziale \ref{r:experiment}. Nie widaæ natomiast wyra¼nej ró¿nicy miêdzy
tymi dwoma sposobami normalizacji. Zaskakuj± s³abe wyniki otrzymane przy u¿yciu ¶redniej wa¿onej - u¿ycie jej obni¿y³o wynik w stosunku do podej¶cia stosuj±cego zwyk³± ¶redni± arytmetyczn±.
...
\end{enumerate}
Dziêki kolejnym modyfikacjom algorytmu uda³o siê ostatecznie poprawiæ jako¶æ predykcji o niemal 10\% w stosunku do modelu bazowego. Warto zaznaczyæ, ¿e mimo dokonanych
